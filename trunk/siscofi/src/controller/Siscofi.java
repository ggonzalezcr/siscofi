...
public class Siscofi {
	...
	// Guarda os guichês e os seus respectivos IPs
	private Map<String, String> guiches;

	// Guarda os controles que estão no Banco de Dados
	private ConcurrentLinkedQueue<Controle> controles;

	// Mapeamento do guichê para o atendente logado no guichê.
	private Map<String, String> guicheAtendente;
	...
	public void iniciarSistema() {
		try {
			...
			guiches = new HashMap<String, String>();
			determinarDadosAtendimento();
			guicheAtendente = new HashMap<String, String>();
	...
	public void gerarXMLFlash(Aviso aviso) throws ServletException {
		// ControleDAO dao = new ControleDAO();
		// XMLUtil.geradorXMLPainel(dao.listaControles(), aviso,
		// AVISO_XML);
	 ...
	 * 
	 * @return True se o guichê está em atividade no sistema e False em caso
	 *         contrário.
	 */
	public String getGuiche(String ip) {
		// return gerenciadorGuiches.existGuiche(guiche);
		String guiche = guiches.get(ip);
		if (guiche == null)
			return "";
		return guiche;
	}
	...
	/**
	 * Verifica se um IP tstá cadastrado.
	 * 
	 * @param ip
	 *            IP a ser atualizado.
	...
	public boolean contemGuiche(String guiche) {
		return guiches.containsValue(guiche);
	}
	...
	public String getIp(String guiche) {
		for (Iterator<String> it = guiches.keySet().iterator(); it.hasNext();) {
			String ip = it.next();
			if (guiches.get(ip).equalsIgnoreCase(guiche))
				return ip;
		}
		return "";
	}
	 ...
	 */
	public void addGuiche(String ip, String guiche) {
		// gerenciadorGuiches.addGuiche(guiche.toUpperCase());
		guiches.put(ip, guiche);
	}
	...
	public void removeGuiche(String ip) {
		try {
			guiches.remove(ip);
			// gerenciadorGuiches.removeGuiche(guiche);
		} catch (Exception e) {
		}
	...
	private String getAtendente(String guiche) {
		return guicheAtendente.get(guiche);
	}

	/**
	 * Registra o login do atendente em no guichê.
	 * 
	 * @param guiche
	 *            guichê.
	 * @param login
	 *            login do atendente.
	 */
	public void registraAtendente(String guiche, String login) {
		guicheAtendente.put(guiche, login);
	}
