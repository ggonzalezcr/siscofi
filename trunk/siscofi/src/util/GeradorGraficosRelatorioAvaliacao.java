...
public class GeradorGraficosRelatorioAvaliacao {
	
	private String graficosPath = Configuracao.getPath() + "/admin/images/";
	
	public GeradorGraficosRelatorioAvaliacao() {
	...
	public void gerarGraficos(DadosAvaliacao dadosAvaliacao) {
		...
		for(int i =1; i <=5 ; i++){
			...
			geraImagemGrafico(pieDataset, graficosPath + "grafico0" + i + ".jpg", dadosAvaliacao.getQuestao(i), i);
		}
	}
	
	private void geraImagemGrafico(DefaultPieDataset pieDataset, String graficoPath, String questao, int indiceQuestao) {
		JFreeChart chart = ChartFactory.createPieChart3D(
	            indiceQuestao+ " - " + questao,  // chart title
	            pieDataset,             // data
	            true,               // include legend
	            true,
	            false
	...
	private DefaultPieDataset createDataSet(String[] alternativas){
		...
		while(cont < alternativas.length){
			int index = alternativas[cont].trim().indexOf(":");
			Double porcentagem = new Double(alternativas[cont].substring(index + 1,alternativas[cont].length() -1));
			pieDataSet.setValue(alternativas[cont], porcentagem);
			cont++;
		}
		return pieDataSet;
	}
	...
	public static void main(String[] args) {
		GeradorGraficosRelatorioAvaliacao ger = new GeradorGraficosRelatorioAvaliacao();
