...
/**
 * Bean que armazena um resumo dos tempos de atendimentos realizados por cada
 * guichê.
 * 
...
public class TempoMedioAtendimento {
	...
	/**
	 * Adiciona os guiches ao mapa de tempo de atendimento ou atualiza o tempo
	 * medio e o ultimo tempo dos guiches existentes
	 * 
	 * @param contagem
	 *            A quantidade de atendimentos
	 * @param variacao
	 *            O fator que determinara o até quanto o tempo médio deve variar
	 * @param tempoMinimo
	 *            O menor tempo possível
	 * @param tempoMaximo
	 *            O maior tempo possível
	 * @param guiche
	 *            O guiche a ser atualizado
	 * @param ultimoTempo
	 *            O tempo a ser acrescentado já ocorreram neste guiche
	 */
	public boolean addTempoAtendimento(int contagem, double variacao,
			double tempoMinimo, double tempoMaximo, String guiche,
			long ultimoTempo) {
		if (tempoAtendimentos.containsKey(guiche)) {
			TempoAtendimento ta = tempoAtendimentos.get(guiche);
			if (ta.contabilizaTempos(contagem, variacao, tempoMinimo,
					tempoMaximo, ultimoTempo)) {
	 ...
	 * 
	 * @param tempoFinal
	 *            O tempo final do atendimento
	 * @param guiche
	 *            O guiche que se esta pesquisando
	...
	public double getTempoMedioGuiches(List<String> listagem) {
		double soma = 0.0;
		int total = listagem.size();
		if (total == 0) {
			return 0;
		} else {
			if (listagem.size() != 0) {
				for (String guiche : listagem) {
					soma += tempoAtendimentos.get(guiche).getTempoMedio();
				}
			}
		}
		return soma / total;
